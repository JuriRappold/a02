...
class HighScore:
    ...
    def __init__(self, player1, opponent):
        if player1 and opponent:
            if isinstance(player1, Player | MockPlayer) and isinstance(
                opponent, Player | Computer | MockPlayer
            ):
                self.chart = {
                    player1.get_username(): player1.get_total_score(),
                    opponent.get_username(): opponent.get_total_score(),
                }
            else:
                raise TypeError(
                    "Not a Player or Computer"
                )  #: player1: {type(player1)}; opponent: {type(opponent)}
        else:
