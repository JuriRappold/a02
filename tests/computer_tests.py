...
def test_init_difficulty_None():
    with pytest.raises(TypeError, match="type is none"):
        test = Computer("Harald", None)

def test_init_difficulty_non_valid_str():
    with pytest.raises(ValueError, match="Value Error: value is a non-valid string"):
        test = Computer("Harald", "idbfv")

def test_init_difficulty_list():
    with pytest.raises(TypeError, match="Typer Error: type is list"):
        test = Computer("Haradl", [1, "apple"])

def test_init_difficulty_tuple():
    with pytest.raises(TypeError, match="Typer Error: type is tuple"):
        test = Computer("Haradl", (1, "apple"))

def test_init_difficulty_dict():
    with pytest.raises(TypeError, match="Typer Error: type is dict"):
        test = Computer("Haradl", {"sup": "nothing"})

def test_init_difficulty_computer():
    with pytest.raises(TypeError, match="Typer Error: type is Computer"):
