...
### Makefile commands
We use makefile for running tests, measure quality of the code, style the code, and autogenerating documentation

#### Environment & Setup
| Target | Purpose/Description |
|--------|---------------------|
| `make version` | Check the current Python executable and version |
| `make venv` | Setup a virtual environment and display activation instructions |
| `make install` | Install packages from requirements.txt |
| `make installed` | List all installed Python packages |

#### Code Quality & Linting
| Target | Purpose/Description |
|--------|---------------------|
| `make pylint` | Run pylint static analysis on program and test files |
| `make flake8` | Run flake8 linting on program and test files |
| `make lint` | Run both flake8 and pylint linters |
| `make black` | Format code using black formatter |
| `make codestyle` | Apply code style formatting (runs black) |
| `make bandit` | Find security issues using bandit |

#### Testing & Coverage
| Target | Purpose/Description |
|--------|---------------------|
| `make unittest` | Run unit tests using pytest |
| `make coverage` | Run tests with code coverage report (HTML output) |
| `make coverage-xml` | Run tests with code coverage report (XML output) |
| `make test` | Run full test suite including linting and coverage |

#### Documentation
| Target | Purpose/Description |
|--------|---------------------|
| `make pydoc` | Generate documentation using pydoc |
| `make pdoc` | Generate documentation using pdoc |
| `make pyreverse` | Generate UML diagrams using pyreverse |
| `make doc` | Generate comprehensive documentation (pdoc and pyreverse) |

## Code Metrics & Statistics
| Target | Purpose/Description |
|--------|---------------------|
| `make radon-cc` | Calculate cyclomatic complexity metrics |
| `make radon-mi` | Calculate maintainability index metrics |
| `make radon-raw` | Calculate raw metrics (LOC, etc.) |
| `make radon-hal` | Calculate Halstead metrics |
| `make cohesion` | Calculate code cohesion metrics |
| `make metrics` | Run all code metrics analysis |

## Cleanup
| Target | Purpose/Description |
|--------|---------------------|
| `make clean` | Cleanup generated files and cache directories |
| `make clean-doc` | Clean documentation files (includes clean) |
| `make clean-all` | Complete cleanup including virtual environment (includes clean and clean-doc) |

### Activating virtual environment

#### To create virtual environment to run tests and pylints, do:
   ```sh
    make venv
   ```
